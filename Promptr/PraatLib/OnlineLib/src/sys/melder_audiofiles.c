...
// The following code was added by Michael Feld to allow for reading of
// sound from memory buffers.

MemFileBuf mfopen(char* rawdata, int length, int copy)
{
	printf("Creating MemFileBuffer of length %i, copy=%i \n", length, copy);
	MemFileBuf buf = malloc(sizeof(structMemFileBuf));
	buf->ptr = 0;
	buf->len = length;
	buf->destroy = copy;
	if (copy) {
		buf->data = malloc(length);
		memcpy(buf->data, rawdata, length);
	} else {
		buf->data = rawdata; // no copy!
	}
	return buf;
}

size_t mfread(void* ptr, size_t size, size_t count, MemFileBuf buf)
{
	int max_el = (int) ((buf->len - buf->ptr) / (int)size);
	if ((int)count > max_el) {
		printf(" ... adjusting number of readable elements (requested: %i) \n", count);
		count = (size_t)max_el;
	}
	if (count <= 0) {
	...
	size_t b = size * count;
	printf(" ... read %i b (%i * %i b) \n", b, count, size);
	memcpy(ptr, buf->data + buf->ptr, b);
	buf->ptr += (int)b;
	return count;
}

int mgetc(MemFileBuf buf)
{
	if (buf->ptr >= buf->len) return EOF;
	char cur = buf->data[buf->ptr];
	buf->ptr ++;
	return (int)cur;
}

void mfclose(MemFileBuf buf)
{
	if (buf == NULL) return;
	printf("Closing MemFileBuffer of length %i, destroy=%i \n", buf->len, buf->destroy);
	if (buf->destroy) free(buf->data);
	free(buf);
}



// The following function is almost identical to "Melder_readAudioToFloat" above, except that
// it works on memory buffers instead of real files.
int Melder_readAudioToFloatB(MemFileBuf f, int numberOfChannels, int encoding, double *leftBuffer, double *rightBuffer, long numberOfSamples) {
	long i;
	int readEverything = 0;
	Melder_assert (sizeof (char) == 1 && sizeof (short) == 2 && sizeof (double) == 8);   // the "double" size checks the Melder_LINEAR_16 memory reuse trick
	switch (encoding) {
		case Melder_LINEAR_8_SIGNED:
			if (numberOfChannels == 1)
				for (i = 1; i <= numberOfSamples; i ++) {
					signed char value;
					if (! mfread (& value, 1, 1, f)) error1 (L"File too small (mono 8-bit).")
					leftBuffer [i] = value * (1.0f / 128);
				}
			else if (rightBuffer)
				for (i = 1; i <= numberOfSamples; i ++) {
					signed char left, right;
					if (! mfread (& left, 1, 1, f) || ! mfread (& right, 1, 1, f)) error1 (L"File too small (stereo 8-bit).")
					leftBuffer [i] = left * (1.0f / 128);
					rightBuffer [i] = right * (1.0f / 128);
				}
			else
		...
		case Melder_LINEAR_8_UNSIGNED:
			if (numberOfChannels == 1)
				for (i = 1; i <= numberOfSamples; i ++)
					leftBuffer [i] = bingetu1B (f) * (1.0f / 128) - 1.0f;
			else if (rightBuffer)
				for (i = 1; i <= numberOfSamples; i ++) {
					leftBuffer [i] = bingetu1B (f) * (1.0f / 128) - 1.0f;
					rightBuffer [i] = bingetu1B (f) * (1.0f / 128) - 1.0f;
				}
			else
				for (i = 1; i <= numberOfSamples; i ++) {
					int left = bingetu1B (f), right = bingetu1B (f);
					leftBuffer [i] = (left + right) * (1.0f / 256) - 0.5f;
				}
			break;
		case Melder_LINEAR_16_BIG_ENDIAN:
			if (numberOfChannels == 1) {
				unsigned char *bytes = (unsigned char *) & leftBuffer [numberOfSamples + 1] - numberOfSamples * 2;   // memory reuse: in bottom quarter
				mfread (bytes, 1, numberOfSamples * 2, f);
				for (i = 1; i <= numberOfSamples; i ++) {
					unsigned char byte1 = * bytes ++, byte2 = * bytes ++;
//printf("   bytes #%i: %i %i\n", i, byte1, byte2);
					long value = (signed short) (((unsigned short) byte1 << 8) | (unsigned short) byte2);
				...
				unsigned char *bytes = (unsigned char *) & leftBuffer [numberOfSamples + 1] - numberOfSamples * 4;   // memory reuse: in bottom half
				mfread (bytes, 1, numberOfSamples * 4, f);
				for (i = 1; i <= numberOfSamples; i ++) {
					unsigned char byte1 = * bytes ++, byte2 = * bytes ++, byte3 = * bytes ++, byte4 = * bytes ++;
					long left = (signed short) (((unsigned short) byte1 << 8) | (unsigned short) byte2);
					long right = (signed short) (((unsigned short) byte3 << 8) | (unsigned short) byte4);
		...
		case Melder_LINEAR_24_BIG_ENDIAN:
			if (numberOfChannels == 1)
				for (i = 1; i <= numberOfSamples; i ++)
					leftBuffer [i] = bingeti3B (f) * (1.0f / 8388608);
			else if (rightBuffer)
				for (i = 1; i <= numberOfSamples; i ++) {
					leftBuffer [i] = bingeti3B (f) * (1.0f / 8388608);
					rightBuffer [i] = bingeti3B (f) * (1.0f / 8388608);
				}
			else
				for (i = 1; i <= numberOfSamples; i ++) {
					long left = bingeti3B (f), right = bingeti3B (f);
					leftBuffer [i] = (left + right) * (1.0f / 16777216);
				}
			break;
		case Melder_LINEAR_24_LITTLE_ENDIAN:
			if (numberOfChannels == 1)
				for (i = 1; i <= numberOfSamples; i ++)
					leftBuffer [i] = bingeti3LEB (f) * (1.0f / 8388608);
			else if (rightBuffer)
				for (i = 1; i <= numberOfSamples; i ++) {
					leftBuffer [i] = bingeti3LEB (f) * (1.0f / 8388608);
					rightBuffer [i] = bingeti3LEB (f) * (1.0f / 8388608);
				}
			else
				for (i = 1; i <= numberOfSamples; i ++) {
					long left = bingeti3LEB (f), right = bingeti3LEB (f);
					leftBuffer [i] = (left + right) * (1.0f / 16777216);
				}
			break;
		case Melder_LINEAR_32_BIG_ENDIAN:
			if (numberOfChannels == 1)
				for (i = 1; i <= numberOfSamples; i ++)
					leftBuffer [i] = bingeti4B (f) * (1.0f / 32768 / 65536);
			else if (rightBuffer)
				for (i = 1; i <= numberOfSamples; i ++) {
					leftBuffer [i] = bingeti4B (f) * (1.0f / 32768 / 65536);
					rightBuffer [i] = bingeti4B (f) * (1.0f / 32768 / 65536);
				}
			else
				for (i = 1; i <= numberOfSamples; i ++) {
					long left = bingeti4B (f), right = bingeti4B (f);
					leftBuffer [i] = (left + right) * (1.0f / 65536 / 65536);
				}
			break;
		case Melder_LINEAR_32_LITTLE_ENDIAN:
			if (numberOfChannels == 1)
				for (i = 1; i <= numberOfSamples; i ++)
					leftBuffer [i] = bingeti4LEB (f) * (1.0f / 32768 / 65536);
			else if (rightBuffer)
				for (i = 1; i <= numberOfSamples; i ++) {
					leftBuffer [i] = bingeti4LEB (f) * (1.0f / 32768 / 65536);
					rightBuffer [i] = bingeti4LEB (f) * (1.0f / 32768 / 65536);
				}
			else
				for (i = 1; i <= numberOfSamples; i ++) {
					long left = bingeti4LEB (f), right = bingeti4LEB (f);
					leftBuffer [i] = (left + right) * (1.0f / 65536 / 65536);
				}
			break;
		case Melder_IEEE_FLOAT_32_BIG_ENDIAN:
			if (numberOfChannels == 1)
				for (i = 1; i <= numberOfSamples; i ++)
					leftBuffer [i] = bingetr4B (f);
			else if (rightBuffer)
				for (i = 1; i <= numberOfSamples; i ++) {
					leftBuffer [i] = bingetr4B (f);
					rightBuffer [i] = bingetr4B (f);
				}
			else
				for (i = 1; i <= numberOfSamples; i ++) {
					double left = bingetr4B (f), right = bingetr4B (f);
					leftBuffer [i] = (left + right) * 0.5;
				}
			break;
		case Melder_IEEE_FLOAT_32_LITTLE_ENDIAN:
			if (numberOfChannels == 1)
				for (i = 1; i <= numberOfSamples; i ++)
					leftBuffer [i] = bingetr4LEB (f);
			else if (rightBuffer)
				for (i = 1; i <= numberOfSamples; i ++) {
					leftBuffer [i] = bingetr4LEB (f);
					rightBuffer [i] = bingetr4LEB (f);
				}
			else
				for (i = 1; i <= numberOfSamples; i ++) {
					double left = bingetr4LEB (f), right = bingetr4LEB (f);
					leftBuffer [i] = (left + right) * 0.5;
				}
			break;
		case Melder_MULAW:
			if (numberOfChannels == 1)
				for (i = 1; i <= numberOfSamples; i ++)
					leftBuffer [i] = ulaw2linear [bingetu1B (f)] * (1.0f / 32768);
			else if (rightBuffer)
				for (i = 1; i <= numberOfSamples; i ++) {
					leftBuffer [i] = ulaw2linear [bingetu1B (f)] * (1.0f / 32768);
					rightBuffer [i] = ulaw2linear [bingetu1B (f)] * (1.0f / 32768);
				}
			else
				for (i = 1; i <= numberOfSamples; i ++) {
					long left = ulaw2linear [bingetu1B (f)], right = ulaw2linear [bingetu1B (f)];
					leftBuffer [i] = (left + right) * (1.0f / 65536);
				}
			break;
		case Melder_ALAW:
			if (numberOfChannels == 1)
				for (i = 1; i <= numberOfSamples; i ++) {
					leftBuffer [i] = alaw2linear [bingetu1B (f)] * (1.0f / 32768); }
			else if (rightBuffer)
				for (i = 1; i <= numberOfSamples; i ++) {
					leftBuffer [i] = alaw2linear [bingetu1B (f)] * (1.0f / 32768);
					rightBuffer [i] = alaw2linear [bingetu1B (f)] * (1.0f / 32768);
				}
			else
				for (i = 1; i <= numberOfSamples; i ++) {
					long left = alaw2linear [bingetu1B (f)], right = alaw2linear [bingetu1B (f)];
					leftBuffer [i] = (left + right) * (1.0f / 65536);
				}
			break;
		case Melder_FLAC_COMPRESSION:
