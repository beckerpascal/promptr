...
unsigned int bingetu1B (MemFileBuf f) { return (unsigned char) mgetc (f); }
...
long bingeti3B (MemFileBuf f) {
	unsigned long result;
	unsigned char bytes [3]; mfread (bytes, 1, 3, f);
	result =
		((unsigned long) bytes [0] << 16) |
		((unsigned long) bytes [1] << 8) | (unsigned long) bytes [2];
	if ((bytes [0] & 128) != 0)
...
long bingeti4B (MemFileBuf f) {
	if (binario_longBE4 && Melder_debug != 18) {
		long l; mfread (& l, 4, 1, f); return l;
	} else {
		unsigned char bytes [4]; mfread (bytes, 1, 4, f); return
			((unsigned long) bytes [0] << 24) | ((unsigned long) bytes [1] << 16) |
			((unsigned long) bytes [2] << 8) | (unsigned long) bytes [3];
	}
}
...
double bingetr4B (MemFileBuf f) {
	if (binario_floatIEEE4msb && Melder_debug != 18) {
		float x; mfread (& x, 4, 1, f); return x;
	} else {
		unsigned char bytes [4];
		double x;
		long exponent;
...
double bingetr4LEB (MemFileBuf f) {
	if (binario_floatIEEE4lsb && Melder_debug != 18) {
		float x; mfread (& x, 4, 1, f); return x;
	} else {
		unsigned char bytes [4];
...
// Memory File Buffer variants added by Michael Feld

// (search for 'MemFileBuf' throughout this file.)
