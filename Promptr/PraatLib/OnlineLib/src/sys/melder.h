...
#ifdef __cplusplus    // case distinction added by Michael Feld
int MelderAudio_stopPlaying (bool);   /* Returns 1 if sound was playing. */
#else
...
typedef struct
{
	int ptr; // offset into "data" (current position marker)
	char* data; // actual buffer contents
	int len; // length of data
	int destroy; // if set to 1, data is destroyed on 'free'
} structMemFileBuf, *MemFileBuf;
